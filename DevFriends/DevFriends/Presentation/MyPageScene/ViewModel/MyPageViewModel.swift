...
extension MyPageViewModel {
    ...
    func showLogout() {
        actions?.showPopup(Popup(title: "로그아웃", message: "정말 로그아웃 하시겠어요?", done: "로그아웃", close: "닫기"))
    }
    
    func showWithdrawl() {
        actions?.showPopup(Popup(title: "탈퇴하기", message: "계정을 삭제하면 개발친구의 모든 활동 정보가 삭제됩니다. 계정 삭제 후 7일간 다시 가입할 수 없습니다.", done: "탈퇴하기", close: "취소"))
    }
}
